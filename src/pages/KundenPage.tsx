...
export default function KundenPage() {
  ...
  const [viewingRecord, setViewingRecord] = useState<Kunden | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getKunden());
    } finally {
      setLoading(false);
    }
  }
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell className="font-medium">{record.fields.organisation ?? '—'}</TableCell>
                <TableCell>{record.fields.ansprechperson_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.ansprechperson_nachname ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Kunden. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
