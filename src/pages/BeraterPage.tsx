...
export default function BeraterPage() {
  const [records, setRecords] = useState<Berater[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Berater | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Berater | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Berater | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBerater());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Berater['fields']) {
    await LivingAppsService.createBeraterEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Berater['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBeraterEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBeraterEntry(deleteTarget.record_id);
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Berater. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
