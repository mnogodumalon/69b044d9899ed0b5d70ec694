...
interface BeraterDialogProps {
  ...
  defaultValues?: Berater['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BeraterDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: BeraterDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'berater');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "kuerzel": string | null, // Kürzel\n  "rolle": LookupValue | null, // Rolle (select one key: "geschaeftsfuehrer" | "berater" | "coach" | "projektentwickler" | "interimsmanager" | "moderator" | "externer_partner") mapping: geschaeftsfuehrer=Geschäftsführer, berater=Berater, coach=Coach, projektentwickler=Projektentwickler, interimsmanager=Interimsmanager, moderator=Moderator, externer_partner=Externer Partner\n  "status": LookupValue | null, // Status (select one key: "aktiv" | "verfuegbar" | "ausgelastet" | "inaktiv") mapping: aktiv=Aktiv, verfuegbar=Verfügbar, ausgelastet=Ausgelastet, inaktiv=Inaktiv\n  "email": string | null, // E-Mail\n  "telefon": string | null, // Telefon\n  "kompetenzen": string | null, // Kompetenzen / Schwerpunkte\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Berater bearbeiten' : 'Berater hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="interimsmanager">Interimsmanager</SelectItem>
                <SelectItem value="moderator">Moderator</SelectItem>
                <SelectItem value="externer_partner">Externer Partner</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="ausgelastet">Ausgelastet</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="kompetenzen">Kompetenzen / Schwerpunkte</Label>
            <Textarea
              id="kompetenzen"
              value={fields.kompetenzen ?? ''}
              onChange={e => setFields(f => ({ ...f, kompetenzen: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
