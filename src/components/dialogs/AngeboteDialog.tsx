...
export function AngeboteDialog({ open, onClose, onSubmit, defaultValues, projekteList, leistungskatalogList, enablePhotoScan = false, enablePhotoLocation = true }: AngeboteDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'angebote');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "angebotsnummer": string | null, // Angebotsnummer\n  "angebotsdatum": string | null, // YYYY-MM-DD\n  "projekt": string | null, // Display name from Projekte (see <available-records>)\n  "leistungen": string | null, // Display name from Leistungskatalog (see <available-records>)\n  "gesamtpreis": number | null, // Gesamtpreis (EUR)\n  "gueltigkeitsdatum": string | null, // YYYY-MM-DD\n  "angebotsstatus": LookupValue | null, // Status (select one key: "entwurf" | "versendet" | "angenommen" | "abgelehnt") mapping: entwurf=Entwurf, versendet=Versendet, angenommen=Angenommen, abgelehnt=Abgelehnt\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["projekt", "leistungen"]);
        ...
        const leistungenName = raw['leistungen'] as string | null;
        if (leistungenName) {
          const leistungenMatch = leistungskatalogList.find(r => matchName(leistungenName!, [String(r.fields.leistungsbezeichnung ?? '')]));
          if (leistungenMatch) merged['leistungen'] = createRecordUrl(APP_IDS.LEISTUNGSKATALOG, leistungenMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues ? 'Angebote bearbeiten' : 'Angebote hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="angebotsdatum"
              type="date"
              value={fields.angebotsdatum ?? ''}
              onChange={e => setFields(f => ({ ...f, angebotsdatum: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="leistungen">Angebotene Leistungen</Label>
            <Select
              value={extractRecordId(fields.leistungen) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, leistungen: v === 'none' ? undefined : createRecordUrl(APP_IDS.LEISTUNGSKATALOG, v) }))}
            >
              <SelectTrigger id="leistungen"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {leistungskatalogList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
                    {r.fields.leistungsbezeichnung ?? r.record_id}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <Label htmlFor="gueltigkeitsdatum">Gültig bis</Label>
            <Input
              id="gueltigkeitsdatum"
              type="date"
              value={fields.gueltigkeitsdatum ?? ''}
              onChange={e => setFields(f => ({ ...f, gueltigkeitsdatum: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="angebotsstatus">Status</Label>
            <Select
              value={lookupKey(fields.angebotsstatus) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, angebotsstatus: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="angebotsstatus"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
