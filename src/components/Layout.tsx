...
import { FileText, FolderKanban, LayoutDashboard, Menu, Receipt, Settings, Users, X } from 'lucide-react';
import { useState, useEffect } from 'react';
import ChatWidget from '@/components/ChatWidget';

// ⚡ Customize these for your app
const APP_TITLE = 'inclou. Unternehmensplattform';
...
const navigation = [
  ...
  { name: 'Berater', href: '/berater', icon: FileText },
  { name: 'Leistungskatalog', href: '/leistungskatalog', icon: FileText },
  ...
  { name: 'Rechnungsliste', href: '/rechnungsliste', icon: Receipt },
];
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside
        className={`
          fixed top-0 left-0 z-50 h-full w-72 bg-sidebar border-r border-sidebar-border overflow-hidden
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-6 border-b border-sidebar-border">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <h1 className="text-sm font-semibold tracking-tight text-sidebar-foreground truncate">{APP_TITLE}</h1>
          </div>
          <button
          ...
          {navigation.map(item => {
            ...
            <NavLink
              ...
              className={({ isActive }: { isActive: boolean }) =>
                `flex items-center gap-2 px-4 py-2 rounded-2xl text-sm transition-colors min-w-0 ${
                  isActive
      ...
      <div className={IS_EMBED ? "" : "lg:pl-72"}>
        {!IS_EMBED && (
